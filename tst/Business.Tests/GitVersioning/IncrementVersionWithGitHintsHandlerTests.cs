...
{
    private List<GitCommit> Commits {get; } =
    [
        new("27d8879", "fake(Build): Updated build.yml to target the template solution file [skip hint]",
            new List<GitCommitFileInfo>()),
        new("27d8880", "build(Pipeline): Downgraded to .net core 3.1 from 5 [skip ci]",
            new List<GitCommitFileInfo>()),
        new("27d8881", "ci(Versioning): Increment version 0.0.0 -> 0.0.0 [skip ci] [skip hint]",
            new List<GitCommitFileInfo>())
    ];

    private IEnumerable<GitCommitVersionInfo> VersionInfos { get; } = new List<GitCommitVersionInfo>
    {
        new("fake", "build", "fake(Build): Updated build.yml to target the template solution file [skip hint]"),
        new("build", "Pipeline", "build(Pipeline): Downgraded to .net core 3.1 from 5 [skip ci]"),
        new("ci", "Versioning", "Increment version 0.0.0 -> 0.0.0 [skip ci] [skip hint]")
    };
